...
\caption{Ansatz circuit used after weighted-RVB initialization.  A depth-$p$ HVA applies four edge-color blocks per layer.  Each block uses one shared parameter and applies Qiskit two-qubit Pauli rotations on every disjoint bond in that color group.  The weighted-RVB initializer is represented as an input state block in this initial report; a hardware-preparable RVB state-preparation circuit is future work.}
