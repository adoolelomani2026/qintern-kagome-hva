...
Figure~\ref{fig:ansatz-circuit} shows the actual circuit structure used for the HVA refinement.  Each color block $G_c$ contains disjoint two-qubit Heisenberg gates, and each Heisenberg gate is implemented in Qiskit as \texttt{RXX}$(2\theta)$, \texttt{RYY}$(2\theta)$, and \texttt{RZZ}$(2\theta)$ on the same bond.  The source implementation is \texttt{src/kagome\_heisenberg\_poc.py::build\_heisenberg\_ansatz}; a compact written version is included in \texttt{docs/ansatz\_circuit.md}.

\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{ansatz_circuit_schematic.png}
\caption{Ansatz circuit used after weighted-RVB initialization.  A depth-$p$ HVA applies four edge-color blocks per layer.  Each block uses one shared parameter and applies Qiskit two-qubit Pauli rotations on every disjoint bond in that color group.}
