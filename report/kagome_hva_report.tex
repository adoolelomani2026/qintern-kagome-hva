...
QWorld Association, QResearch Department}
\date{July 2026}
...
The spin-$1/2$ Kagome antiferromagnetic Heisenberg model is a canonical frustrated-magnetism benchmark and a central model in the search for quantum spin liquid (QSL) behavior.  Large-scale DMRG studies support a gapped topological spin-liquid ground state and place the thermodynamic-limit energy near $-0.4386(5)$ per site in the physical spin convention \cite{yan2011,depenbrock2012}.  The open-boundary 19-site patch studied here is small enough for exact fixed-sector diagonalization yet large enough to expose limitations of purely local dimer product states.
...
Figure~\ref{fig:workflow-flowchart} gives the project flow as two connected branches.  The exact-reference branch diagonalizes the fixed 19-site Kagome Hamiltonian to obtain $E_0$ and $|\psi_0\rangle$.  The ansatz branch constructs a signed weighted-RVB initializer, refines it with edge-colored Heisenberg-HVA layers, and compares the resulting trial state with the exact reference through energy, fidelity, and correlation diagnostics.  The dashed calibration-scan box is included only as design feedback for future local HVA blocks.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{workflow_flowchart.png}
\caption{Project schematic.  The target Hamiltonian is fixed throughout.  The upper branch produces the exact reference state, and the lower branch produces the no-calibration variational trial state through weighted-RVB initialization and Heisenberg-HVA refinement.  Calibration scans are shown with a dashed box because they are future ansatz-design guidance, not a replacement for the present no-calibration workflow.}
\label{fig:workflow-flowchart}
\end{figure}

\section{Provenance and Scope}

The original Ahsan Kagome-VQE material is used as provenance and methodology context, not as code copied into this workflow.  In particular, it provides the source cross-check for the 19-site Kagome connectivity, context for hardware-efficient Qiskit VQE experiments, and the calibrated-Hamiltonian or defect-triangle strategy described in Ref.~\cite{ahsan2026}.  The present proof-of-concept instead develops a separate no-calibration route based on signed weighted-RVB initialization and shallow edge-colored Heisenberg-HVA refinement.
...
When discussing the original calibrated-Hamiltonian direction, we use cautious language: defect or triangle couplings are increased above the uniform value, with reported calibrated values depending on the patch, Hamiltonian convention, defect set, and scan.  In the Ahsan workflow this idea appears as defect-triangle Hamiltonian engineering with selected bonds near $J'\approx 2$ and a hardware-efficient RealAmplitudes-style VQE circuit \cite{ahsan2026}.  This report states precise calibration values only for scans reproduced in the current repository.
...
\]
In the rounded convention used in the Ahsan calibration table, the corresponding 19-site value appears as approximately $-29.14$ \cite{ahsan2026}.
...
Figure~\ref{fig:ansatz_circuit} makes the circuit explicit.  One HVA layer consists of four edge-color blocks.  A color block applies the same variational angle to all disjoint bonds in that color group.  In Qiskit, each interaction
\[
\exp[-i\theta(X_iX_j+Y_iY_j+Z_iZ_j)]
\]
is implemented as \texttt{RXX}$(2\theta)$, \texttt{RYY}$(2\theta)$, and \texttt{RZZ}$(2\theta)$ on the same qubit pair.  The implementation is \texttt{src/kagome\_heisenberg\_poc.py::build\_heisenberg\_ansatz}, and a compact standalone circuit description is included in \texttt{docs/ansatz\_circuit.md}.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{ansatz_circuit_schematic.png}
\caption{Block-level edge-colored Heisenberg-HVA ansatz circuit.  The weighted-RVB initializer is followed by repeated color blocks $G_c(\theta_{\ell,c})$.  Each $G_c$ block represents several parallel two-qubit Heisenberg interactions on disjoint bonds, decomposed into Qiskit Pauli rotation gates $R_{XX}(2\theta)$, $R_{YY}(2\theta)$, and $R_{ZZ}(2\theta)$.  A full 19-qubit gate-level circuit would contain 120 Heisenberg bond gates, or 360 $R_{XX}/R_{YY}/R_{ZZ}$ rotations, for the $p=4$ run, so the full gate-level diagram is omitted for readability.}
...
\end{equation}
Large $P_{\rm AF}$ indicates more distributed antiferromagnetic correlation weight.  Motivated by the DMRG diagnostics of Ref.~\cite{depenbrock2012}, the code also reports the normalized participation $P_{\rm AF}/|E|$, the fraction of antiferromagnetic weight trapped on nearly perfect singlet bonds with $C_{ij}<-2$, and a graph-distance spin-correlation profile.  The fitted graph-distance decay length is only a finite-patch proxy; it is not a thermodynamic correlation length.  The figures use a deterministic graph layout for visualization, not a physical Kagome embedding.
...
\begin{table}[H]
\centering
\caption{Paper-inspired correlation diagnostics.  The frozen AF fraction measures how much antiferromagnetic bond weight lies on nearly perfect dimers.  The graph-distance decay length is a compact finite-patch proxy inspired by the log-linear correlation plots of Ref.~\cite{depenbrock2012}.}
\label{tab:paper_diagnostics}
\begin{tabular}{lrrr}
\toprule
State & $P_{\rm AF}/30$ & Frozen AF frac. & $\xi_{\rm graph}$ \\
\midrule
Static dimer & 0.300 & 1.000 & -- \\
Weighted RVB-54 & 0.681 & 0.000 & 1.662 \\
Weighted RVB + HVA $p=4$ & 0.681 & 0.000 & 1.624 \\
Exact & 0.682 & 0.000 & 1.356 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.68\textwidth]{spin_distance_profile.png}
\caption{Finite-patch spin-correlation decay proxy.  Correlations are averaged by graph distance because reliable physical coordinates for the extracted 19-site patch are not part of the input data.  The static dimer has only nearest-neighbor singlet correlations, while the weighted-RVB and HVA-refined states show distributed short-range correlations closer to the exact-state pattern.}
\label{fig:spin_distance}
\end{figure}
...
\begin{table}[H]
\centering
\caption{No-calibration HVA compared with reproduced calibrated-Hamiltonian exact reference states.  Calibrated rows are exact ground states of modified Hamiltonians evaluated against the original target Hamiltonian.}
\label{tab:calibration}
\begin{tabular}{lrr}
\toprule
Method / reference state & Energy error & Fidelity \\
\midrule
...
Figure~\ref{fig:calibration} plots the same comparison as a landscape.  The horizontal axis is target energy error, $|E-E_{\rm exact}|$, so moving left means better energy agreement with the original 19-site Hamiltonian.  The vertical axis is fidelity with the exact target state, so moving up means better wavefunction agreement.  The best region is therefore the upper-left corner.

The colored scan points in Fig.~\ref{fig:calibration} are exact ground states of modified Hamiltonians: a bond, edge-color group, or triangle has coupling $J'$ instead of the uniform value.  The red star is the current no-calibration weighted-RVB + HVA $p=4$ state.  This distinction matters.  The figure does not show that the present no-calibration circuit beats calibration.  Instead, it shows that small local Hamiltonian changes, especially on triangles, can produce exact reference states extremely close to the target.  Those local structures are useful clues for future HVA blocks, while the target Hamiltonian used for evaluation remains unchanged.
...
\end{minipage}
\caption{Calibration scan landscape.  Left means lower target-energy error and up means higher exact-state fidelity, so the upper-left corner is best.  The colored points are exact ground states of modified Hamiltonians; the red star is the present no-calibration weighted-RVB + HVA $p=4$ result.  The figure motivates calibration-informed ansatz design, not a claim that the current HVA already outperforms calibrated reference states.}
...
\item \texttt{results/19site\_paper\_diagnostics.csv}: paper-inspired correlation summaries.
\item \texttt{results/19site\_spin\_distance\_profile.csv}: graph-distance spin-correlation profile used in Fig.~\ref{fig:spin_distance}.
\item \texttt{results/literature\_benchmark\_comparison.csv}: convention conversion against Depenbrock DMRG and Ahsan rounded values.
...
\bibitem{ahsan2026}
M. Ahsan,
``Utility-scale Experimental Quantum Computation with Hardware Efficient Ans\"{a}tze and Calibrated Hamiltonian,''
